...
const wasmUrl = window.freedoomWasmUrl || 'doom.wasm';
WebAssembly.instantiateStreaming(fetch(wasmUrl), importObject)
    ...
    window.freedoomInput = function(keyCode, pressed) {
        (pressed ? keyDown : keyUp)(keyCode);
    };
    ...
    /*Unified touch, pen and mouse input for the product controls.*/
    [["enterButton", 13],
     ["leftButton", 0xac],
     ["rightButton", 0xae],
     ["upButton", 0xad],
    ...
    canvas.focus();
    printFocusInHint();
    document.getElementById("runtime-status").innerText = "ENGINE READY";
    window.freedoomRuntimeReady = true;
    ...
    document.getElementById("runtime-status").innerText = "ENGINE ERROR";
    window.freedoomRuntimeError = String(error);
    console.error("Freedoom engine failed to start", error);
});
